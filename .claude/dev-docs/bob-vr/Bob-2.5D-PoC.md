...
### 5.4 Godot 2.5D parallax — ЧАСТИЧНО валидирован

| Компонент | Статус | Результат |
|-----------|--------|----------|
| ParallaxBackground + ParallaxLayer | OK | Слои загружаются и рендерятся корректно |
| Camera2D breathing | OK | Двойная синусоида (6s + 10s), органичное движение |
| Runtime PNG загрузка | OK | `Image.load()` + `ImageTexture.create_from_image()` |
| Масштабирование спрайтов | OK | Автоматический fit to viewport |
| **Parallax-эффект глубины** | **FAIL** | См. ниже |

**Файлы:**
- Сцена: `godot/scenes/parallax_2d.tscn`
- Скрипт: `godot/scripts/parallax_scene.gd`
- Ассеты: `godot/assets/parallax/layer_{0..3}_*.png`

**Фундаментальная проблема параллакса:**

Для видимого эффекта глубины слои должны двигаться с РАЗНОЙ скоростью
(motion_scale: 0.2, 0.5, 0.8, 1.0). Но Bob занимает несколько слоёв глубины
(его тело — от переднего до среднего плана). При разных скоростях
Bob "разъезжается" — разные части тела движутся с разной скоростью.

При near-uniform motion_scale (0.97–1.01) Bob выглядит целым, но параллакс-эффект
полностью исчезает — сцена выглядит как обычное 2D изображение.

**Решение:** Генерировать фон БЕЗ Bob, разделять на слои ТОЛЬКО фон,
Bob — отдельный спрайт поверх. Но это требует решения проблемы
identity+scene consistency (см. раздел 5.5).

### 5.5 Композиция Bob + фон — НЕРЕШЁННАЯ ПРОБЛЕМА

**Суть:** Для полноценного 2.5D нужны одновременно:
1. **Сохранение айдентики Bob** — Kontext отлично справляется, но принимает только 1 изображение
2. **Согласованность со сценой** — Bob должен сидеть в КОНКРЕТНОМ кресле из сгенерированной сцены

Ни одна локальная модель не умеет делать оба одновременно.

**Протестированные подходы:**

| Подход | Инструмент | Результат |
|--------|-----------|----------|
| Kontext (1 изображение) | mflux-generate-kontext | Айдентика OK, но не видит сцену |
| Qwen-Image-Edit q4 | mflux-generate-qwen-edit | Мозаичные артефакты, непригоден |
| Qwen-Image-Edit full | mflux-generate-qwen-edit | OOM killed (54 GB > 64 GB RAM) |
| Qwen-Image-Edit q8 | mflux-generate-qwen-edit | 66 мин, айдентика полностью потеряна |

**Непротестированные варианты (на следующую сессию):**

| Подход | Сложность | Перспективность |
|--------|-----------|----------------|
| Kontext side-by-side hack (склейка 2 изображений в одно) | Низкая (~10 мин тест) | Средняя — может не работать |
| LoRA на Bob (mflux-train) → fill с LoRA | Средняя (one-time train) | Высокая — identity baked in model |

**Обнаруженный тулкит mflux:**

В ходе исследования найдены полезные команды mflux:
- `mflux-generate-fill` — inpainting по маске (вставка объектов в сцену)
- `mflux-train` — LoRA finetuning (обучение на изображениях Bob)
- `rembg` с `isnet-anime` — вырезка cartoon-персонажей из фона

**Целевая архитектура (когда identity+scene проблема решена):**
- AI генерирует отдельные ассеты (фон, мебель, Bob)
- Godot композирует их в 2.5D сцену
- Каждый элемент можно менять независимо

Подробности: DecisionLog.md, D-018
